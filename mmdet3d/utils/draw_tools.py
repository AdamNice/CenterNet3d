...
class Object3d(object):
    ...
    def __init__(self, label_file_line):
        ...
        self.w = data[8]  # box height
        self.l = data[9]  # box width
        self.h = data[10]  # box length (in meters)
        ...
        try:
            ...
            self.score = 1.
        self.box3d = np.array(
                [data[11], data[12], data[13], data[8], data[9], data[10], data[14]]).astype(np.float32)
...
def draw_lidar(pc, fig=None, color=(1,1,1), scale=2., axis=True, show=False):
    ...
    #               figure=fig)
    mlab.points3d(pc[:, 0], pc[:, 1], pc[:, 2], color=color, mode='point', scale_factor=scale,
                  figure=fig)
    if axis:
        # draw origin
...
def draw_gt_boxes3d(gt_boxes3d, fig, color=(0,1,0), line_width=1, draw_text=False, scores=None, text_scale=(1,1,1), color_list=None, show=False):
