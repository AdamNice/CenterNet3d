        ...
        if coors.size(-1) == 3:
            ...
            for i in range(batch_size):
                ...
                voxel, voxel_coor = self.forward_single(
                    points[inds], coors[inds][:, 1:])  #voxels:(N,C) voxel_coor:(N,3)
                coor_pad = nn.functional.pad(
                    voxel_coor, (1, 0), mode='constant', value=i)
                voxel_coors.append(coor_pad)
                voxels.append(voxel)
            features = torch.cat(voxels, dim=0)   #features:(N,C)
            feature_coors = torch.cat(voxel_coors, dim=0) #feature_coors:(N,4)
