...
class DynamicVFE(nn.Module):
    ...
    def forward(self,
        ...
        if self._with_cluster_center:
            voxel_mean, mean_coors = self.cluster_scatter(features, coors) #每个voxel内点的均值，对应voxel的坐标
            points_mean = self.map_voxel_center_to_point(
                coors, voxel_mean, mean_coors)   #将体素的特征赋给对应的体素内所有的点
            # TODO: maybe also do cluster for reflectivity
            f_cluster = features[:, :3] - points_mean[:, :3]
            features_ls.append(f_cluster) #将每个点的与聚类中心的偏移作为附加特征
        ...
        if self._with_voxel_center:
            ...
            features_ls.append(f_center) #将每个点与体素中心的偏移作为附加特征
        ...
        for i, vfe in enumerate(self.vfe_layers):  #vfe中MLP特征提取是针对所有点进行的
            point_feats = vfe(features)
            if (i == len(self.vfe_layers) - 1 and self.fusion_layer is not None
                    and img_feats is not None):
                point_feats = self.fusion_layer(img_feats, points, point_feats,
                                                img_metas)
            voxel_feats, voxel_coors = self.vfe_scatter(point_feats, coors) #基于每个点的体素坐标，通过maxpooling提取体素的特征
            if i != len(self.vfe_layers) - 1:
                # need to concat voxel feats if it is not the last vfe
                feat_per_point = self.map_voxel_center_to_point(
                    coors, voxel_feats, voxel_coors) #获取每个点对应的体素的特征
